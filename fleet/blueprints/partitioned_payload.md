...
state.json        fmt, status (ok|reseed_pending|failed), per APPEND-ONLY journal (players, gear, abilities)
                  ...
                  frozen, published, reg (the §2.6 counts), counts_stale (set by a day rebuild with the
                  day's own checkpoint, cleared when reg is recomputed -- §6.2-3)}; pins snapshot +
                  pins.mirrored_sha (§2.5); projection {rules_sha,
...
2. **Rebuild dirty days, newest first, at most 8 per run** (a mass invalidation drains over
   ...
   **across all days**; a loser found in *another* day marks that day dirty (restoring its
   ...
   the winner's day records the event. **The neighbour mark is durable with the collapse
   itself**: the sig/loser rows commit in sqlite before `state.json` is saved (the per-day
   checkpoint order), so a kill between the two keeps the winner's day dirty while the
   neighbour's mark is lost — and the winner's rebuild re-derives it: every loser recorded
   in *another* day whose winner is a run of this day and whose copy that day's `keys.npz`
   still holds is a neighbour again (`RunDB.losers_won_by_day`; idempotent, since a rebuilt
   neighbour no longer holds the loser; one `state.invalidations` record per collapse
   event). `PARTS_TEST_CRASH_AT=day:after_commit:<n>` stands in that window. **The queue is
   re-derived after every completed
...
3. **Window-level, every run** (numpy over the ≤ 24 **local** window caches — nothing is
   ...
   in which case `seq` does not advance and `built` is unchanged. **The per-week counts
   (`weeks[].reg`, §2.6) are not part of this stage**: `Builder.week_counts()` runs before it
   every run and recomputes, from `thin.npz`, every week whose `state.weeks[w].counts_stale`
   is set (by `build_day`, for the weeks the day touched before and after, saved with the
   day's own checkpoint) or whose counts are absent, and clears the flag — independent of the
   fingerprint and of the deadline, so a run that ends between the per-day checkpoints and
   this stage (the builder's own deadline stop reusing the previous window artifacts,
   `parts.window_stale=1`, or a kill) still publishes counts equal to a row scan. An
   in-memory "touched weeks" set would die with that run and leave the counts stale for good.
...
by the new builder. The two stores the per-day checkpoint writes in sequence — sqlite, then
`state.json` — hold no fact that only the later one carries: what a lost state save drops
(the day's clean mark, a collapse neighbour's dirty mark, a week's stale-counts mark) is
re-derived by the rebuild the still-dirty day forces (§6.2-2) or was written in the same
save as the mark that forces it (§6.2-3), so a kill at any instant converges to the replay.
...
## Appendix F — Revision 6 (2026-09-02, PR-1 stage B round 4): changes and the blockers they answer

| # | change | sections | answers |
|---|---|---|---|
| 45 | Cross-day collapse neighbour durable with the collapse: `_collapse` re-derives, from the committed loser rows (`RunDB.losers_won_by_day`), every neighbour whose day's `keys.npz` still holds the loser, so the winner day's rebuild after a kill between the per-day checkpoint's sqlite commit and its state save marks the neighbour again; one `state.invalidations` record per collapse event; `PARTS_TEST_CRASH_AT=day:after_commit:<n>`; the replay-kill case in `test_incremental_idempotent` | §6.1, §6.2-2, §6.3, §9.1 | round-3 incremental blocker 1 (the loser copy `Zrevmid999999999/1` served forever from day 241 after a SIGKILL inside day 240's checkpoint: d241 1555 rows vs 1550, week 33 re-cubed under the wrong sha, no collapse invalidation, a third run a no-op) |
| 46 | Per-week counts: `state.weeks[w].counts_stale` set by `build_day` (old and new weeks of the day, saved with the day's checkpoint) and consumed by `Builder.week_counts()`, which runs **every run before the window stage**, independent of the window fingerprint and of the deadline (the in-memory `touched_weeks` set is gone); `parts.weeks_recomputed=` in health; asserted in `test_incremental_idempotent` after the deadline-stopped run, after every crash resume and after a kill between step 2 and 3 drained at the cap | §2.6, §6.1, §6.2-3, §6.3, §9.1 | round-3 incremental blocker 2 (`weeks[36].reg` US 340 / EU 405 / CN 125 / KR 10 instead of 885 / 1080 / 140 / 15 after a `--deadline 34` drain; after a kill, US 105 and no EU/CN/KR entry at all — the current reset's chips under-counting per region for good) |
